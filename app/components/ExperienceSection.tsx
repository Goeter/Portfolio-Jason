...
export default function ExperienceSection() {
  ...
  const nextSlide = () => {
    setCurrentIndex((prev) => (prev + 1) % Math.max(1, projects.length - cardsPerView + 1))
  }

  const prevSlide = () => {
    setCurrentIndex(
      (prev) =>
        (prev - 1 + Math.max(1, projects.length - cardsPerView + 1)) % Math.max(1, projects.length - cardsPerView + 1),
    )
  }
